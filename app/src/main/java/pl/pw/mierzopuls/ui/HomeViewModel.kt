...
import pl.pw.mierzopuls.model.plus
import pl.pw.mierzopuls.model.sendEvent
import pl.pw.mierzopuls.model.sortByDate
...
class HomeViewModel(
    ...
    var enterInformation by mutableStateOf(false)
    ...
    private fun finishStudy(timeStamps: List<Long>, values: List<Double>) {
        ...
        processSignal(
            values.toList(),
            timeStampsOffSetFromZero,
            UserInfo(name, age, gender = "", height, weight)
        ).let { study ->
